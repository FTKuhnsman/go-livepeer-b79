...
func (w *wizard) setMaxFaceValue() {
        fmt.Printf("Enter new maximum ticket face value in Wei")
        mfv := w.readBigInt()

        val := url.Values{
                "facevaluelimit": {fmt.Sprintf("%v", mfv.String())},
        }

        httpPostWithParams(fmt.Sprintf("http://%v:%v/setFaceValueLimit", w.host, w.httpPort), val)
}
